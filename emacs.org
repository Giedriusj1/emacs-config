...
** ORG mode
#+BEGIN_SRC emacs-lisp
(define-key tab-map (kbd "o") 'hydra-tab-org/body)
...
(defhydra hydra-c (:color blue)
  ( "j" helm-yas-complete "helm yas complete")
  )
...
(defvar swift-command-map
  (let ((map (make-keymap)))
    ...
    (define-key map (kbd "o") 'swift-2-up)
    (define-key map (kbd "l") 'swift-2-down)
...
#+BEGIN_SRC emacs-lisp
(require 'paradox)
(customize-set-variable 'paradox-github-token t)
