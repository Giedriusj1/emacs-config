...
(custom-set-faces '(mode-line-inactive ((t (:background "#333333")))))
#+END_SRC
...
(define-key control-semi-map (kbd "C-d") 'toggle-swift-mode)

(defvar swift-command-map
  (let ((map (make-keymap)))
    (define-key map (kbd "i") 'scroll-down-line)
    (define-key map (kbd "k") 'scroll-up-line)
