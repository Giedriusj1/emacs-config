...
#+begin_src emacs-lisp
(yafolding-mode)
(bind-key* "C-f" 'yafolding-toggle-element)
#+end_src
...
(defun helm-grep (targets &optional recurse zgrep exts)
...
(defun projectile-helm-grep-root ()
  (interactive)

  (projectile-with-default-dir (projectile-project-root)
    (call-interactively (helm-do-grep-1 
                      nil
			 '(10)
			 nil
			 '("*")
...
(define-key projectile-command-map (kbd "a") 'projectile-helm-grep-root)
...
(add-hook 'eshell-load-hook 
	  (lambda()
      (eshell-fringe-status-mode t)
	 ))

(bind-key* "C-`" 'eshell)
;;(bind-key* "C-0" 'multi-term)
#+end_src
